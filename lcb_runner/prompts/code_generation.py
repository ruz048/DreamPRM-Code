...
from lcb_runner.prompts.prompt_constants import PromptConstants

def get_generic_question_template_answer(question: CodeGenerationProblem, cof: str = ""):
    prompt = ""
    if cof:
        # Select appropriate COF based on platform
        prompt += PromptConstants.get_cof_for_problem(question) + "\n"
...
def get_oaireason_question_template_answer(question: CodeGenerationProblem, cof: str = ""):
    prompt = ""
    if cof:
        # Select appropriate COF based on platform
        prompt += PromptConstants.get_cof_for_problem(question) + "\n"
...
def get_codeqwen_question_template_answer(question: CodeGenerationProblem, cof: str):
    prompt = "You will be given a question (problem specification) and will generate a correct Python program that matches the specification and passes all tests. You will NOT return anything except for the program.\n\n"
    if cof:
        # Select appropriate COF based on platform
        prompt += PromptConstants.get_cof_for_problem(question) + "\n"
...
def format_prompt_generation(
    question: CodeGenerationProblem, LanguageModelStyle: LMStyle, cof: str
) -> str:
    if LanguageModelStyle in [
        ...
        chat_messages += [
            {
                "role": "user",
                "content": get_generic_question_template_answer(question, cof),
            },
        ]
        ...
        chat_messages = [
            {
                ...
                "content": PromptConstants.SYSTEM_MESSAGE_GENERIC
                + "\n\n"
                + get_generic_question_template_answer(question, cof),
            },
        ]
        return chat_messages
        ...
        chat_messages = [
            {
                ...
                "content": PromptConstants.SYSTEM_MESSAGE_GENERIC
                + "\n\n"
                + get_oaireason_question_template_answer(question, cof),
            },
        ]
        return chat_messages
    ...
    if LanguageModelStyle == LMStyle.CodeQwenInstruct:
        ...
        prompt += f"{get_codeqwen_question_template_answer(question, cof)}"
        return prompt
