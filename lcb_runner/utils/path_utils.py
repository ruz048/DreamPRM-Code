...
def get_output_path(model_repr:str, args) -> str:
    ...
    cof_suffix = "cof_"+args.cof if args.cof else ""
    prm_data_suffix = "_prm_data" if args.generate_funprm_training_data else ""
    path = f"output/{model_repr}/{scenario}_{n}_{temperature}{cot_suffix}_{cof_suffix}{prm_data_suffix}.json"
    ensure_dir(path)
    return path
