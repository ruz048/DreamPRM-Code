...
class BaseRunner(ABC):
    ...
    def run_main_repair(self, benchmark: list, format_prompt: callable) -> list[list[str]]:
        ...
        with open(
            f"output/{self.model.model_repr}/{Scenario.codegeneration}_{self.args.codegen_n}_{self.args.temperature}__eval_all.json"
        ) as f:
    ...
    def run_main(self, benchmark: list, format_prompt: callable) -> list[list[str]]:
        ...
        prompts = [
            format_prompt(problem, self.model.model_style, self.args.cof) for problem in benchmark
        ]
